...
Consider the 43,879th row of the ImageNet 1k training dataset. It appears to be a picture of a lionfish:
...
Note that sequence order matters here. Just like with text, the meaning changes if the patches are shuffled.
...
We can see in the model card that this model has been trained for zero shot image classification on the ImageNet dataset.
...
We have learned about vision transformers (likely used in modern LLMs vision encoders).
We saw how images are encoded by splitting them into patches, linearly embedding them, adding position embeddings, creating a sequence of vectors.
We got a glimpse of how representation allows the model to understand and classify images, and perform other tasks performed by vision capable LLMs.
